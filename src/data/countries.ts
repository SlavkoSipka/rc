/**
 * Every country and territory the shop can ship to, with its ISO 3166-1
 * alpha-2 code and international dialling code.
 *
 * Single source for three things that used to be maintained separately and
 * had drifted apart: the checkout's country dropdown, the phone prefix it
 * fills in, and the country code sent to PayPal with the shipping address.
 *
 * Names already used by the shop are kept verbatim so existing orders keep
 * matching (for example "Turkey", not the newer "Turkiye"). Uninhabited
 * territories with no postal service are left out.
 */
...
export const COUNTRIES: Country[] = [
  ...
  { name: 'Saint Helena, Ascension and Tristan da Cunha', iso2: 'SH', dialCode: '+290' },
  ...
  { name: 'United States Virgin Islands', iso2: 'VI', dialCode: '+1340' },
...
/** International dialling prefix, e.g. "+852". Falls back to the US prefix. */
export function getDialCode(country: string): string {
  return BY_NAME.get(country)?.dialCode ?? '+1';
}

/** ISO 3166-1 alpha-2 code PayPal expects, e.g. "HK". */
export function getCountryCode(country: string): string {
  return BY_NAME.get(country)?.iso2 ?? 'US';
}
